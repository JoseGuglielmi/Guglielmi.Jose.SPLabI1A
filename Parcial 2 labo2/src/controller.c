...
int controller_filtrarGenero(LinkedList *lista, LinkedList *filter)
{
	...
	if (lista != NULL)
	{
		...
		do
		{
			switch (menuFiltrarPorGenero())
			{

			case 1:
				filter = ll_filter(lista, eMovies_fDrama);
				controller_saveAsText("filterDrama.csv", filter);
				break;
			case 2:
				filter = ll_filter(lista, eMovies_fComedia);
				controller_saveAsText("filterComedia.csv", filter);
				break;
			case 3:
				filter = ll_filter(lista, eMovies_fAccion);
				controller_saveAsText("filterAccion.csv", filter);
				break;
			case 4:
				filter = ll_filter(lista, eMovies_fTerror);
